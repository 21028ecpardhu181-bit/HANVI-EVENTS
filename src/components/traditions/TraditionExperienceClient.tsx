...
import { CulturalTheme, culturalThemes } from '@/lib/theme/themeEngine';
...
export interface TraditionData {
  exp: any;
  religionKey: 'hindu' | 'christian' | 'muslim';
  theme: CulturalTheme;
  journeySteps: any[];
  journeySubtitle: string;
}

interface TraditionExperienceClientProps {
  initialSlug: string;
  traditionsMap: Record<string, TraditionData>;
  rawWhatsApp: string;
}

const TRADITION_TABS = [
  { id: 'hindu', label: 'Hindu', name: 'Vedic Mandap' },
  { id: 'christian', label: 'Christian', name: 'Cathedral & Beach' },
  { id: 'muslim', label: 'Muslim', name: 'Nikah & Walima' },
] as const;

export function TraditionExperienceClient({
  initialSlug,
  traditionsMap,
  rawWhatsApp,
}: TraditionExperienceClientProps) {
  // Determine active religion key from initialSlug
  const getInitialKey = (slug: string): 'hindu' | 'christian' | 'muslim' => {
  ...
  const [activeKey, setActiveKey] = useState<'hindu' | 'christian' | 'muslim'>(
    getInitialKey(initialSlug)
  );

  const currentData = traditionsMap[activeKey] || traditionsMap['hindu'];
  const { exp, theme, journeySteps, journeySubtitle } = currentData;

  const handleSwitchTradition = (key: 'hindu' | 'christian' | 'muslim') => {
    if (key === activeKey) return;
    setActiveKey(key);
    // Update URL smoothly without full SSR reload
    window.history.pushState(null, '', `/wedding-experiences/${key}`);
  };

  // Sync if user uses browser Back / Forward buttons
  useEffect(() => {
    const onPopState = () => {
      const path = window.location.pathname;
      setActiveKey(getInitialKey(path));
    };
  ...
  const whatsappInquiryUrl = `https://wa.me/${rawWhatsApp}?text=${encodeURIComponent(
    `Hi Kala Prasad garu, I am interested in planning a ${exp.title} with Hanvi Events. Please share package details and consultation availability.`
  )}`;

  const otherTraditions = TRADITION_TABS.filter((t) => t.id !== activeKey);

  return (
    <main style={{ backgroundColor: theme.bgMain }} className="min-h-screen pt-14 sm:pt-0 transition-colors duration-500">
      
      {/* Chapter 1: Opening Scene (Cinematic Hero) */}
      <section className={`relative w-full py-8 sm:py-16 md:py-24 bg-gradient-to-b ${theme.bgHeroGradient} text-[#FCF9F5] overflow-hidden transition-all duration-500`}>
        ...
        <div className="relative z-10 max-w-[1280px] mx-auto px-3 sm:px-6 md:px-8">
          ...
          {/* Top Breadcrumb & Smooth Tradition Switcher */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 sm:mb-8">
            <Link
              href="/wedding-experiences"
              className="inline-flex items-center gap-1.5 text-xs font-sans-ui uppercase tracking-wider text-[#B88A44] hover:text-[#FCF9F5] transition-colors self-start"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>All Traditions</span>
            </Link>

            {/* Premium Animated Segmented Switcher */}
            <div className="inline-flex items-center p-1 rounded-full bg-black/50 backdrop-blur-md border border-white/20 shadow-lg self-center sm:self-auto">
              {TRADITION_TABS.map((tab) => {
                ...
                return (
                  <button
                    key={tab.id}
                    type="button"
                    onClick={() => handleSwitchTradition(tab.id)}
                    className={`relative px-4 sm:px-5 py-1.5 sm:py-2 rounded-full text-xs font-sans-ui uppercase tracking-wider transition-colors duration-200 cursor-pointer min-h-[34px] flex items-center justify-center font-medium ${
                      isActive ? 'text-white font-semibold' : 'text-white/60 hover:text-white'
                    }`}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="activeTraditionIndicator"
                        transition={{ type: 'spring', stiffness: 450, damping: 35 }}
                        className="absolute inset-0 rounded-full bg-gradient-to-r from-[#B88A44] via-[#C99C55] to-[#A07635] shadow-md border border-[#F5ECDD]/30"
                      />
                    )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-w-2xl mx-auto mt-3">
                  {otherTraditions.map((trad) => (
                    <button
                      key={trad.id}
                      type="button"
                      onClick={() => handleSwitchTradition(trad.id as any)}
                      className="p-3 sm:p-4 rounded-xl bg-[#F5ECDD]/40 border border-[#E8DDCD] hover:border-[#B88A44] hover:bg-[#F5ECDD] transition-all flex items-center justify-between group cursor-pointer text-left w-full"
                    >
                      <div>
      ...
      <section id="book-experience" className="py-10 sm:py-20 bg-[#1F1915] text-[#FCF9F5] text-center border-t border-[#B88A44]/30">
